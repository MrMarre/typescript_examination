import { bookConverter } from "./interfaces.js";
...
const fetchBooks = async () => {
    try {
        ...
        const bookData = data.map((book) => bookConverter(book));
        console.log(data);
        return bookData;
    }
    catch (error) {
...
const displayDetails = (book) => {
    ...
    for (const key in book) {
        if (book[key].elementType) {
            const { className, textInFrontOf, elementType, value } = book[key];
